...
class PathPriorityFilter:
    ...
    def __init__(self, reference_prefixes: List[str] = None):
        self.reference_prefixes = reference_prefixes or []
    
    def evaluate(self, group: DuplicateGroup) -> List[FilterDecision]:
        decisions = []
        for f in group.files:
            is_ref_path = any(f.path.startswith(p) for p in self.reference_prefixes)
            if is_ref_path:
    ...
    def _matching_prefix(self, path: str) -> str:
        for p in self.reference_prefixes:
            if path.startswith(p):
                return p
        return ""
    
    def get_params(self) -> Dict:
        return {"reference_prefixes": self.reference_prefixes}
    
    @classmethod
    def from_params(cls, params: Dict) -> 'PathPriorityFilter':
        return cls(params.get("reference_prefixes", []))
        ...
        for f, depth in depths:
            if depth == best_depth:
                ...
                decisions.append(FilterDecision(
                    action=FilterAction.SKIP,
                    confidence=0.0,
                    reason=f"Deeper path: {depth} vs {best_depth}",
                    filter_name=self.name
                ))
        return decisions
